...
char mmc_readpartialblock( uint32_t address, uint16_t count, uint8_t *sector ) {
	uint16_t al = address & 0x0000FFFF;
	uint16_t ah = (address >> 16) & 0x0000FFFF;
	uint8_t r1 =  mmc_command(0x51, 0xFF, ah, al);
	for (al = 0; al < 50000; al++) {
	...
	if (r1 != 0) return 1;
	while(spi_transfer(0xFF) != 0xFE);
	for(al=0; al < count; al++) sector[al] = spi_transfer(0xFF);
	for(;al<512;al++) spi_transfer(0xFF);
	spi_transfer(0xFF);
	spi_transfer(0xFF);
	return 0;
}
