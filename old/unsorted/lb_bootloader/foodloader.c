...
#   ifdef BOOTLOADER_JUMPER
            ...
            (((BOOTLOADER_PIN >> BOOTLOADER_PINNUM) & 1) == 0) ||                //goes to bootloader state if the pin descibed by BOOTLOADER_PIN and BOOTLOADER_PINNUM is pulled to ground. applies if BOOTLOADER_JUMPER is defined
#   endif
#   ifdef BOOTLOADER_CHAR
            /* 2) or activation via char */
            wait_for_char() ||                                                    //goes to bootloader state if the bootloader entry char is received at startup. applies if BOOTLOADER_CHAR is defined
#   endif
            0) {
