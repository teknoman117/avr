...
#
# $Id: mfile.tcl,v 1.41 2004/07/15 20:51:08 j Exp $
...
set devicelist \
    {at90s1200 at90s2313 at90s2323 at90s2333 at90s2343 \
	 ...
	 {atmega64 0x801100} \
	 {atmega8515 0x800260} \
	 atmega8535 attiny11 attiny12 attiny15 attiny22 \
	 attiny26 attiny28 at94K at76c711 at43usb320 at43usb355 \
	 at86rf401}
...
proc parsetemplate {fname} {
	...
	if {[regexp -- {-Tdata=([0-9a-fA-Fx]+).*__heap_end=([0-9a-fA-Fx]+)} \
		$x dummy y1 y2]} {
...
proc makemfilemenu {} {
	...
	"windows" {
	    set portlist {com1 com2 com3 com4 lpt1 lpt2 lpt3}
	}
	"unix" {
	    switch $tcl_platform(os) {
		"Linux" {
		    set portlist {/dev/ttyS0 /dev/ttyS1 /dev/ttyS2 /dev/ttyS3 \
				      /dev/parport0 /dev/parport1 /dev/parport2}
		}
		"FreeBSD" {
		    set portlist {/dev/cuaa0 /dev/cuaa1 /dev/cuaa2 /dev/cuaa3 \
				      /dev/ppi0 /dev/ppi1 /dev/ppi2}
		}
		"Solaris" {
		    set portlist {/dev/ttya /dev/ttyb}
		}
	    }
	}
    }
	...
	foreach p {alf avr910 avrisp bascom bsd \
...
$Revision: 1.41 $ $Date: 2004/07/15 20:51:08 $
...
proc setextmemopts {} {
	...
	"vars" {
	    set line "EXTMEMOPTS = -Wl,-Tdata=$xstart,--defsym=__heap_end=$xend"
	}
	"heap" {
