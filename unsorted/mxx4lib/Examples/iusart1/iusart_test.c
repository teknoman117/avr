...
#define RBUFFER1_SIZE 256   //set the receive buffer size to 256 bytes
...
int main ( void ) {
	...
	init_iusart1 (51, DB8 | P_N | SB1);             //51 is the ubrr value for 19.2 kBaud at 16 MHz
	writei1((uint8_t*)"Test for Interrupt Driven Uart\r", 31);  //write starting message
	writei1((uint8_t*)"As you enter data it is displayed\r", 34);  //write informational message
	while (1) {                                           //repeat forever
		sprintf(str, "Bytes in buffer: %d; Write Pointer: %d, Read Pointer: %d; Last Character: %c\r", bytesInBuf(), wp, rp, (char) rxbi0()); //create this string with the sprinf command in stdio
                writei1((uint8_t*)str, strlen(str));      //write the string
